...
GradeBook::GradeBook() : size(0), capacity(5) {
...
void GradeBook::showStudent(int i) {
    if (i >= 0 && i < size)
        students[i].display();
}

void GradeBook::saveToFile(string filename) {
...
void GradeBook::loadFromFile(string filename) {
